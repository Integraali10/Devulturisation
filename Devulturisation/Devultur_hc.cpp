  ...
  *(hreal) = cinfo.image_height;
  ...
  if (pich != NULL) {
    ...
    for (int i = 0; i < dctsize2; i++) {
      ...
      std::cout << std::setw(4) << quants[0]->quantval[i] << ' ';
      if ((i + 1) % 8 == 0) {
        std::cout << std::endl;
      }
    }
...
void main() {
  // reading cl file
  FILE * ptrFile = fopen("Devultur_cl_uchar.cl", "rb");
 ...
 if (INTELWEWANTDEBUG){
   ...
   clBuildProgram(program, 1, &device, "-g -s C:\\Users\\Savva\\Source\\Repos\\DevulturisationCL\\Devulturisation\\Devultur_cl_uchar.cl", NULL, NULL);
  }
  else
  {
  ...
  //////FINDING KERNELS/////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////// 
  cl_kernel kernel, kernelvert, kernelhori, kernelnook, kerneldivi, kerneldevnull;
  kernel = clCreateKernel(program, "devultur_most", &err6);
  kernelvert = clCreateKernel(program, "devultur_vert", &err6);
  kernelhori = clCreateKernel(program, "devultur_hori", &err6);
  kernelnook = clCreateKernel(program, "devultur_nook", &err6);
  kerneldivi = clCreateKernel(program, "devultur_division", &err6);
  kerneldevnull = clCreateKernel(program, "devultur_devnull", &err6);
  //////////////////////////////////////////////////////////////////////
  //////reading file////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////
  char *filename = "expgrad_20.jpg";
  u8 *pich = NULL;
  float fquant[dctsize2], iquant[dctsize2];
  int w, h, wreal, hreal;
  read_JPEG_file(filename, pich, &w, &wreal, &h, &hreal, fquant, iquant); //to-do - add color component or write new function for that

  pich = new u8[w*h];

  read_JPEG_file(filename, pich, &w, &wreal, &h, &hreal, fquant, iquant);
