...
void jpeg_idct_float(const float *inptr, uchar *output_buf, int output_stride)
{
...
void jpeg_fdct_float(float *outptr, __local const uchar *input_buf, int input_stride)
{
  float tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7;
  float tmp10, tmp11, tmp12, tmp13;
...
#pragma unroll
  for (ctr = 0; ctr < DCTSIZE; ctr++) {
    tmp0 = convert_float(input_buf[0] + input_buf[7]);
    tmp7 = convert_float(input_buf[0] - input_buf[7]);
    tmp1 = convert_float(input_buf[1] + input_buf[6]);
    tmp6 = convert_float(input_buf[1] - input_buf[6]);
    tmp2 = convert_float(input_buf[2] + input_buf[5]);
    tmp5 = convert_float(input_buf[2] - input_buf[5]);
    tmp3 = convert_float(input_buf[3] + input_buf[4]);
    tmp4 = convert_float(input_buf[3] - input_buf[4]);
...
__kernel void devultur_most(const __global uchar* pic, __global ushort* res, const __global float *fquant, const __global float *iquant, int rsize, int offset)
{
  ...
  uchar ctblock2[DCTSIZE2];

  __local	uchar tot[LOSI*LOSI];
  __local float fintot[LOSI*LOSI];

  //а тут - запись из глобальной ? Без сознания же писалось
  uchar4 tmptot1 = vload4(0, pic + (gr + lrn)*rsize + gc + lcn * 4 + offset);
  vstore4(tmptot1, 0, &tot[lrn*LOSI + lcn * 4]);
  vstore4((float4)(0), 0, &fintot[lrn*LOSI + lcn * 4]);
  barrier(CLK_LOCAL_MEM_FENCE);

  //2. DCT-QUANT-IDCT
  jpeg_fdct_float(ctblock1, &tot[lr*LOSI + lc], LOSI);
#pragma unroll
  for (int i = 0; i < DCTSIZE2; i++) {
    ctblock1[i] = round(ctblock1[i] * fquant[i]) * iquant[i];
  }
  jpeg_idct_float(ctblock1, ctblock2, DCTSIZE);
...
#pragma unroll
    ...
    {
      fintot[(y + lr)*LOSI + z + lc] += (float)ctblock2[y * DCTSIZE + z];
      barrier(CLK_LOCAL_MEM_FENCE);
    }
  }
...
__kernel void devultur_vert(const __global uchar* pic, __global ushort* res, const __global float *fquant, const __global float *iquant, int rsize, int csize, int offset)
{
  ...
  __local	uchar tot[DCTSIZE*LOSI];
  __local float fintot[DCTSIZE*LOSI];

  uchar8 tmptot1 = vload8(0, pic + (gr + lr)*rsize + offset);
  vstore8(tmptot1, 0, &tot[lr * 8]);
  tmptot1 = vload8(0, pic + (gr + lr + 8)* rsize + offset);
  vstore8(tmptot1, 0, &tot[lr * 8 + DCTSIZE2]);
  vstore16((float16)(0), 0, &fintot[lr * 16]);
  barrier(CLK_LOCAL_MEM_FENCE);

  jpeg_fdct_float(ctblock1, &tot[lr * 8], DCTSIZE);
#pragma unroll
  for (int i = 0; i < DCTSIZE2; i++) {
    ctblock1[i] = round(ctblock1[i] * fquant[i]) * iquant[i];
  }
...
#pragma unroll
    for (int z = 0; z < DCTSIZE; z++)
    {
      fintot[lr * 8 + z + y * 8] += (float)ctblock2[y * 8 + z];
    }
    barrier(CLK_LOCAL_MEM_FENCE);
  }

  ushort8 tmptotfin1 = vload8(0, res + (gr + lr)*rsize + offset) + convert_ushort8_sat_rtz(vload8(0, &fintot[lr * 8]));
  vstore8(tmptotfin1, 0, res + (gr + lr)*rsize + offset);
  tmptotfin1 = vload8(0, res + (gr + lr + 8)* rsize + offset) + convert_ushort8_sat_rtz(vload8(0, &fintot[lr * 8 + DCTSIZE2]));
...
__kernel void devultur_hori(const __global uchar* pic, __global ushort* res, const __global float *fquant, const __global float *iquant, int rsize, int csize, int offset)
{
  ...
  uchar8 tmptot1 = vload8(0, pic + gc + lr*rsize + offset);
  vstore8(tmptot1, 0, &tot[lr * LOSI]);
  tmptot1 = vload8(0, pic + gc + 8 + lr*rsize + offset);
  vstore8(tmptot1, 0, &tot[lr * LOSI + DCTSIZE]);
  vstore16((float16)(0), 0, &fintot[lr * 16]);
  barrier(CLK_LOCAL_MEM_FENCE);

  jpeg_fdct_float(ctblock1, &tot[lr], LOSI);
#pragma unroll
  for (int i = 0; i < DCTSIZE2; i++) {
    ctblock1[i] = round(ctblock1[i] * fquant[i]) * iquant[i];
  }
...
#pragma unroll
    for (int z = 0; z < DCTSIZE; z++)
    {
      fintot[lr + z + y * LOSI] += (float)ctblock2[y * 8 + z];
      barrier(CLK_LOCAL_MEM_FENCE);
    }
  }

  ushort8 tmptotfin1 = vload8(0, res + gc + lr*rsize + offset) + convert_ushort8_sat_rtz(vload8(0, &fintot[lr*LOSI]));
  vstore8(tmptotfin1, 0, res + gc + lr*rsize + offset);
  tmptotfin1 = vload8(0, res + gc + 8 + lr*rsize + offset) + convert_ushort8_sat_rtz(vload8(0, &fintot[lr * LOSI + DCTSIZE]));
